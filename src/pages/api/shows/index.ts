...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getShowsPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'show');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.show.count(countOptions as unknown),
  ...
  async function getShows() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.show
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'show'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
