...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['ticket-seller', 'theater-manager', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CinemaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/cinemas` : null),
    () => getCinemas({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Cinemas',
      path: isTenantUser ? `/cinemas/view/${data?.[0]?.id}` : '/cinemas',
      entity: 'cinema',
    ...
    { name: 'Shows', path: '/shows', entity: 'show', service: AccessServiceEnum.PROJECT, icon: FiBox },
