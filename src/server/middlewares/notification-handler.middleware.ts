...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'ticket_purchase.create': {
    roles: ['theater-manager', 'ticket-seller', 'customer-service-representative'],
    key: 'ticket_purchase',
    tenantPath: [],
    userPath: [],
  },
  'ticket_cancellation.delete': {
    roles: ['theater-manager', 'ticket-seller', 'customer-service-representative'],
    key: 'ticket_cancellation',
    tenantPath: [],
    userPath: [],
  },
  'show_schedule_change.update': {
    roles: ['theater-manager', 'ticket-seller', 'customer-service-representative'],
    key: 'show_schedule_change',
    tenantPath: [],
    userPath: [],
  },
};

const ownerRoles: string[] = ['ticket-seller'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['theater-manager', 'ticket-seller', 'customer-service-representative'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.cinema.findFirst({
